# src/test_graph.py - Test suite for the puzzle chain finder
import os
import time
import logging
import tempfile
...
def test_simple_chain():
    ...
    chain_numbers = [
        next((p.puzzle_number for p in puzzles if p.id == i), "INVALID") for i in chain
    ]
    ...
    logger.info(
        ...
        if chain_numbers == expected
        else "Chain order incorrect!"
    )
...
def test_performance():
    ...
    data_path = os.path.join(
        os.path.dirname(os.path.dirname(__file__)), "data", "source.txt"
    )
    ...
    try:
        ...
        expected_count = sum(
            1 for line in lines if line.strip() and len(line.strip()) == 6
        )
        ...
        if loaded_puzzles != expected_count:
            logger.warning(
                f"Expected {expected_count} puzzles but loaded {loaded_puzzles}"
            )
        ...
        if len(chain) > 0:
            ...
            for i in range(len(chain) - 1):
                ...
                if p1.puzzle_sides["gives"] != p2.puzzle_sides["takes"]:
                    logger.error(
                        f"Invalid connection: {p1.puzzle_number} gives {p1.puzzle_sides['gives']} but {p2.puzzle_number} takes {p2.puzzle_sides['takes']}"
                    )
                    is_valid = False
            ...
            for puzzle_id in chain:
                if puzzle_id in seen:
                    logger.error(f"Duplicate puzzle in chain: {puzzle_id}")
                    duplicates = True
                    break
                seen.add(puzzle_id)
            ...
            if is_valid and not duplicates:
                logger.info(
                    "Chain validation passed: All connections are valid with no duplicates"
                )
            else:
...
def run_all_tests():
    ...
    try:
        ...
        return False  # This return statement was missing
